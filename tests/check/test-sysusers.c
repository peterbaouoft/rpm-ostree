...
{
  /* Now we do the comparison between the converted / generated output to expected */ 
...
static void
setup_sysuser_group (gchar     **test_content,
...
static void
setup_sysuser_passwd(gchar     **test_content,
                     GPtrArray **out_entries,
                     GError    **error)
{
  gboolean ret;
...
test_passwd_conversion(void)
{
  ...
  setup_sysuser_passwd((gchar**) test_passwd, &sysusers_entries, &error);
...
test_group_conversion(void)
{
  ...
  setup_sysuser_group ((gchar**) test_group, &sysusers_entries, &error);
  /* Check Hashtable properties */
  g_assert (sysusers_entries);
      ...
      char *argv[])
{
  ...
  g_test_add_func ("/sysusers/sysuser_conversion", test_sysuser_conversion);
  return g_test_run ();
}
